...
function getAxiosErrorMessage(error: unknown): string {
    if (axios.isAxiosError(error)) {
        ...
        return axiosError.response?.data?.error?.[0]?.message || axiosError.response?.data?.message || axiosError.message;
    }
...
function authorsToText(libro: IndexLibroInput): string | undefined {
    ...
    return libro.authors
        .map((author: unknown) => {
            ...
            if (typeof author === 'object' && author && 'fullName' in (author as any)) return (author as any).fullName;
            if (typeof author === 'object' && author && '_id' in (author as any)) return (author as any)._id?.toString();
...
export function buildLibroText(libro: IndexLibroInput): string {
    ...
    return [`Título: ${libro.title || 'Sin título'}`, `ISBN: ${libro.isbn || 'Sin ISBN'}`, `Autor: ${autor}`, `Categoría: ${categoria}`, `Tipo: ${type}`, `Precio: ${precio}`, `Estado: ${estado}`].join('\n');
}
...
export async function ensureLibroSchema(): Promise<void> {
    try {
        ...
        if (!axios.isAxiosError(error) || error.response?.status !== 404) {
            throw new Error(`No se pudo comprobar el esquema de Weaviate: ${getAxiosErrorMessage(error)}`);
        }
    }
...
export async function findLibroObjectByMongoId(libroId: string): Promise<LibroVectorSearchResult | null> {
    const query = `
        {
            Get {
                ${LIBRO_CLASS_NAME}(
...
export async function upsertLibro(libro: IndexLibroInput, vector: number[]): Promise<LibroVectorSearchResult> {
...
export async function searchLibrosByVector(vector: number[], limit = 5, includeDeleted = false): Promise<LibroVectorSearchResult[]> {
    await ensureLibroSchema();
