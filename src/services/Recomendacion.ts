...
function normalizeRequestContext(context: Array<string | RecommendationContextItem> = []): RecommendationContextItem[] {
    return context
        .map((item) => {
...
async function searchMongoContext(query: string, limit: number, includeDeleted: boolean): Promise<RecommendationContextItem[]> {
    ...
    if (libros.length === 0) {
        ...
        libros = await Libro.find(fallbackFilter).sort({ createdAt: -1 }).limit(limit).populate('authors', 'fullName');
    }
...
export async function recomendarLibros(input: RecomendacionInput): Promise<RecomendacionResult> {
    ...
    const contextItems = requestContext.length > 0 ? requestContext : await searchMongoContext(query, limit, includeDeleted);
    const generation: GenerateTextResult = await AIService.generateRecommendation(query, contextItems);
