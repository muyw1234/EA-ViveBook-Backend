...
const solicitarReserva = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!libro.owner) {
            return sendError(res, 'Este libro no tiene propietario registrado y no se puede reservar', 'Bad Request', 400);
        }
        ...
        if (existingActive) {
            return sendError(res, 'Ya tienes una reserva pendiente o aceptada para este libro', 'Bad Request', 400);
        }
